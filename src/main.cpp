...
#define TFT_CS 15   // Pin 3
#define TFT_RST 4   // Pin 4
#define TFT_DC 2    // Pin 5
#define TFT_MOSI 23 // Pin 6
#define TFT_SCK 18  // Pin 7
#define TFT_MISO 19 // Pin 9

Arduino_DataBus *bus = new Arduino_ESP32SPI(TFT_DC, TFT_CS, TFT_SCK, TFT_MOSI, GFX_NOT_DEFINED);
Arduino_GFX *gfx = new Arduino_ST7789(bus, TFT_RST, 1, false, 240, 320, 0, 0, 0, 0);
#define CANVAS
...
void setup()
{
  ...
  else
  {
    ...
    ui_init();
    lv_obj_add_event_cb(objects.on_off_pwm, pwmcontor, LV_EVENT_VALUE_CHANGED, NULL);
    lv_obj_add_event_cb(objects.pwmpercent, pwmcontor, LV_EVENT_VALUE_CHANGED, NULL);
    lv_obj_add_event_cb(objects.onpump_bt, event_handler, LV_EVENT_CLICKED, pLED2);
    lv_obj_add_event_cb(objects.offpump_bt, event_handler, LV_EVENT_CLICKED, pLED2);
    lv_obj_add_event_cb(objects.adcpage_bt, event_handler, LV_EVENT_CLICKED, NULL);
    lv_obj_add_event_cb(objects.home_bt, event_handler, LV_EVENT_CLICKED, NULL);
    lv_timer_create(get_sw_state, 100, pSW1);
    lv_timer_create(update_adc_chart, 500, NULL);
